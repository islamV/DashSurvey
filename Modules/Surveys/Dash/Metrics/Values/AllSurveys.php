...
class AllSurveys extends Value{
     ...
     */
      public function calc(){
        return
        $this->count( Survey::class ) // or sum // $this->sum(YourModel::class,'id') | id column is optional
    //    ->at('created_at') // optional
      ->column(3) // optional
       // ->href(dash('resource/YourResourceOr Other Links'))
        ->icon('<i class="fa-solid fa-chart-column"></i>') // icon by fontawesome or other | optional
        ->title(__('survey.survey')) // optional
       // ->subTitle('Your subTitle') // optional
       //  ->textBody('Text In Body') // optional
     ->prefix('<i class="fa-solid fa-chart-column"></i> ') // add prefix before number or icon
