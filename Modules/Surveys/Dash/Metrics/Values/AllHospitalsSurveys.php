...
class AllHospitalsSurveys extends Value{
     ...
     */
      public function calc(){
        ...
        $this->count(Survey::class , function($q){
            return $q->where('service_type' ,'hospitals');
        }) // or sum // $this->sum(YourModel::class,'id') | id column is optional
        ->at('created_at') // optional
        ->column(3) // optional
        ->href(dash('resource/HospitalsSurvey'))
        ->icon('<i class="fa-solid fa-circle-h"></i>') // icon by fontawesome or other | optional
         ->title(__('survey.hospitalsReports')) // optional
        // ->subTitle('Your subTitle') // optional
        //  ->textBody('Text In Body') // optional
         ->prefix('<i class="fa-solid fa-circle-h"></i>') // add prefix before number or icon
